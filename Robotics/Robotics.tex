% TODO:
% - some (many) images ans schemes are missing
% - check typos and syntax after chapter 5
% - 
...
% ? - can probably be explained further/better
...
\begin{itemize}
    ...
    % todo: check the condition in which control law is performed in practical cases: while (?), whilst (?), solely (?), uniquely (?)
...
% todo: check if IMP means "Iterative Motion Planning" or anything else, and explain it
...
Mechanical devices properly installed on the wrist of the robot, called RCC (Remote Center of Compliance) able to react to external forces by moving the center of compliance (defined as the point in space where linear forces correspond to pure linear displacements and torques correspond to pure rotation) from the gripper of the robot to the hole where the gripped objects should be placed. With the RCC, the forces generated by any misalignment move the object in a way that corrects the misalignment problem, rather than exacerbating it.
\begin{itemize}
    \item advantages: low cost, simple devices
    \item drawbacks: low flexibility, use limited to specific applications
\end{itemize}.
...
When a task involves at the same time both motion and force application, it often happens that the directions along which forces are applied, are different with respect to directions along which motion takes place.
